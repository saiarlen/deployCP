...
func (h *AppHandler) ManageUpdateRuntime(c *fiber.Ctx) error {
	...
	if err := h.service.UpdateRuntimeSettings(c.Context(), id, processManager, workers, workerClass, maxMemory, timeout, execMode, restartPolicy, healthPath, port, runtimeVersion, runtimeApply, currentUserID(c), c.IP()); err != nil {
		h.base.Sessions.SetFlash(c, err.Error())
...
func (h *AppHandler) ManageUpdateRedisPassword(c *fiber.Ctx) error {
	id, err := repositories.ParseID(c.Params("id"))
	if err != nil {
		return c.Status(400).SendString(err.Error())
	}
	...
	app, _, err := h.appRedisItem(id, rid)
	if err != nil {
		h.base.Sessions.SetFlash(c, err.Error())
		return c.Redirect(platformURLWithTab("app", id, "databases"))
	}
